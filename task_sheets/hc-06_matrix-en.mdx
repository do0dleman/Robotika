...
> Arduino Bluetooth Controller gamepad sends a letter corresponding to the button pressed (in this case 'B' for bottom) and '0' on button release. This is used in the code above to read command without using `BTSerial.readString` to avoid high input lag.
