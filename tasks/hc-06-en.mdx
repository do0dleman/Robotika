...
To communicate with HC-06 using smartphone we will use Arduino Bluetooth Controller ([paly market](https://play.google.com/store/apps/details?id=com.giumig.apps.bluetoothserialmonitor)).

In this tasks you will only use HC-06 to receive data from the phone, so you won't need to connect RXD port to Arduino. However, if you want to send data to a connected device or want to configure HC-06 using [AT command,](https://www.instructables.com/AT-command-mode-of-HC-05-Bluetooth-module/) you will have to use RXD pin.
